...
function MediaUpload({label,hint,value,cover=false,onUpload,onClear}){return <div className="admin-media-field"><span>{label}</span>{value?<div className={`admin-media-preview ${cover?'cover':''}`}><img src={value} alt="Yüklenen görsel önizlemesi"/><button type="button" onClick={onClear}><X/> Kaldır</button></div>:<div className="admin-media-empty"><ImagePlus/> Görsel seçilmedi</div>}<label className="admin-secondary"><UploadCloud/> Görsel yükle<input type="file" accept="image/jpeg,image/png,image/webp,image/avif" onChange={onUpload}/></label><small>{hint}</small></div>}
...
export default function AdminPanel({supabaseUrl='',supabaseKey=''}){
 ...
 const [brandDraft,setBrandDraft]=useState({id:null,name:'',slug:'',description:'',logo_url:'',cover_image_url:'',meta_title:'',meta_description:'',sort_order:0,is_active:true});
 const [categoryDraft,setCategoryDraft]=useState({id:null,name:'',slug:'',description:'',image_url:'',meta_title:'',meta_description:'',sort_order:0,is_active:true});
 ...
 async function uploadLookupImage(kind,field,draft,setDraft,event){
  const file=event.target.files?.[0];event.target.value='';if(!file)return;
  if(file.size>5*1024*1024){flash('Görsel en fazla 5 MB olabilir.',true);return}
  setBusy(true);
  try{
   const ext=(file.name.split('.').pop()||'webp').toLowerCase().replace(/[^a-z0-9]/g,'');
   const name=slugify(draft.slug||draft.name||kind)||kind,path=`${kind}/${name}-${Date.now()}.${ext}`;
   const uploaded=await supabase.storage.from('product-images').upload(path,file,{cacheControl:'31536000',upsert:false});
   if(uploaded.error){flash(`Görsel yüklenemedi: ${uploaded.error.message}`,true);return}
   const {data}=supabase.storage.from('product-images').getPublicUrl(path);
   setDraft(current=>({...current,[field]:data.publicUrl}));flash('Görsel yüklendi. Şimdi kaydet düğmesine basın.');
  }catch(uploadError){flash(`Görsel yüklenemedi: ${uploadError.message}`,true)}finally{setBusy(false)}
 }
 ...
 async function saveBrand(event){
  event.preventDefault();setBusy(true);const payload={...brandDraft,name:clean(brandDraft.name),slug:slugify(brandDraft.slug||brandDraft.name),description:clean(brandDraft.description),logo_url:brandDraft.logo_url||null,cover_image_url:brandDraft.cover_image_url||null};delete payload.id;
  ...
  setBusy(false);if(result.error){flash(result.error.message.includes('cover_image_url')?'Önce Supabase görsel alanı güncellemesini çalıştırın.':result.error.message,true);return}setBrandDraft({id:null,name:'',slug:'',description:'',logo_url:'',cover_image_url:'',meta_title:'',meta_description:'',sort_order:0,is_active:true});await loadData();flash('Marka kaydedildi.');
 }

 async function saveCategory(event){
  event.preventDefault();setBusy(true);const payload={...categoryDraft,name:clean(categoryDraft.name),slug:slugify(categoryDraft.slug||categoryDraft.name),description:clean(categoryDraft.description),image_url:categoryDraft.image_url||null};delete payload.id;
  ...
  setBusy(false);if(result.error){flash(result.error.message.includes('image_url')?'Önce Supabase görsel alanı güncellemesini çalıştırın.':result.error.message,true);return}setCategoryDraft({id:null,name:'',slug:'',description:'',image_url:'',meta_title:'',meta_description:'',sort_order:0,is_active:true});await loadData();flash('Kategori kaydedildi.');
 }
 ...
 return <main className="admin-shell">
  ...
  <div className="admin-layout">
   ...
   <section className="admin-content">
    ...
    {active==='brands'&&<><div className="admin-heading"><div><p>MARKALAR</p><h1>Marka yönetimi</h1><span>Marka sayfaları, logo, kapak görseli ve SEO metinlerini buradan yönetin.</span></div></div><div className="admin-lookup-layout"><form className="admin-form admin-lookup-form" onSubmit={saveBrand}><h2>{brandDraft.id?'Markayı düzenle':'Yeni marka'}</h2><Field label="Marka adı"><input value={brandDraft.name} onChange={e=>setBrandDraft({...brandDraft,name:e.target.value,slug:brandDraft.slug||slugify(e.target.value)})} required/></Field><Field label="Bağlantı adı"><input value={brandDraft.slug} onChange={e=>setBrandDraft({...brandDraft,slug:slugify(e.target.value)})} required/></Field><Field label="Marka açıklaması"><textarea rows="5" value={brandDraft.description||''} onChange={e=>setBrandDraft({...brandDraft,description:e.target.value})}/></Field><MediaUpload label="Marka logosu" hint="Şeffaf arka planlı WebP veya PNG; önerilen 600 × 300 px." value={brandDraft.logo_url} onUpload={e=>uploadLookupImage('brands','logo_url',brandDraft,setBrandDraft,e)} onClear={()=>setBrandDraft({...brandDraft,logo_url:''})}/><MediaUpload label="Marka kapak görseli" hint="Marka sayfası üst alanı için WebP; önerilen 1600 × 700 px." value={brandDraft.cover_image_url} cover onUpload={e=>uploadLookupImage('brands','cover_image_url',brandDraft,setBrandDraft,e)} onClear={()=>setBrandDraft({...brandDraft,cover_image_url:''})}/><Field label="SEO başlığı"><input value={brandDraft.meta_title||''} onChange={e=>setBrandDraft({...brandDraft,meta_title:e.target.value})}/></Field><Field label="SEO açıklaması"><textarea rows="3" value={brandDraft.meta_description||''} onChange={e=>setBrandDraft({...brandDraft,meta_description:e.target.value})}/></Field><button className="admin-primary" disabled={busy}><Save/> {busy?'Kaydediliyor…':'Markayı kaydet'}</button></form><div className="admin-lookup-list">{brands.map(item=><article key={item.id}><div>{item.logo_url&&<img className="admin-lookup-thumb" src={item.logo_url} alt=""/>}<b>{item.name}</b><small>/{item.slug}</small><p>{item.description}</p></div><span><button className="admin-icon" onClick={()=>setBrandDraft({...item,logo_url:item.logo_url||'',cover_image_url:item.cover_image_url||''})}><Pencil/></button><button className="admin-icon danger" onClick={()=>deleteLookup('brands',item)}><Trash2/></button></span></article>)}</div></div></>}

    {active==='categories'&&<><div className="admin-heading"><div><p>KATEGORİLER</p><h1>Kategori yönetimi</h1><span>Ana sayfa blok görsellerini, koleksiyon metinlerini ve SEO bilgilerini yönetin.</span></div></div><div className="admin-lookup-layout"><form className="admin-form admin-lookup-form" onSubmit={saveCategory}><h2>{categoryDraft.id?'Kategoriyi düzenle':'Yeni kategori'}</h2><Field label="Kategori adı"><input value={categoryDraft.name} onChange={e=>setCategoryDraft({...categoryDraft,name:e.target.value,slug:categoryDraft.slug||slugify(e.target.value)})} required/></Field><Field label="Bağlantı adı"><input value={categoryDraft.slug} onChange={e=>setCategoryDraft({...categoryDraft,slug:slugify(e.target.value)})} required/></Field><Field label="Kategori açıklaması"><textarea rows="5" value={categoryDraft.description||''} onChange={e=>setCategoryDraft({...categoryDraft,description:e.target.value})}/></Field><MediaUpload label="Ana sayfa kategori görseli" hint="Erkek, Kadın, Unisex ve Set blokları için dikey WebP; önerilen 1200 × 1500 px." value={categoryDraft.image_url} cover onUpload={e=>uploadLookupImage('categories','image_url',categoryDraft,setCategoryDraft,e)} onClear={()=>setCategoryDraft({...categoryDraft,image_url:''})}/><Field label="SEO başlığı"><input value={categoryDraft.meta_title||''} onChange={e=>setCategoryDraft({...categoryDraft,meta_title:e.target.value})}/></Field><Field label="SEO açıklaması"><textarea rows="3" value={categoryDraft.meta_description||''} onChange={e=>setCategoryDraft({...categoryDraft,meta_description:e.target.value})}/></Field><button className="admin-primary" disabled={busy}><Save/> {busy?'Kaydediliyor…':'Kategoriyi kaydet'}</button></form><div className="admin-lookup-list">{categories.map(item=><article key={item.id}><div>{item.image_url&&<img className="admin-lookup-thumb category" src={item.image_url} alt=""/>}<b>{item.name}</b><small>/{item.slug}</small><p>{item.description}</p></div><span><button className="admin-icon" onClick={()=>setCategoryDraft({...item,image_url:item.image_url||''})}><Pencil/></button><button className="admin-icon danger" onClick={()=>deleteLookup('categories',item)}><Trash2/></button></span></article>)}</div></div></>}
   </section>
  </div>
 </main>;
}
