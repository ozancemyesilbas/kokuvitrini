...
export default function Header(){
 ...
 const results=q.length>1?products.filter(p=>(p.name+' '+p.brand+' '+p.family).toLocaleLowerCase('tr').includes(q.toLocaleLowerCase('tr'))).slice(0,6):[];
 return <>
  ...
  <header className="site-header">
   ...
   <div className="header-actions"><button onClick={()=>setSearch(true)} aria-label="Ara"><Search/></button><a href="/hesabim" aria-label="Hesabım"><UserRound/></a><button onClick={()=>setOpen(true)} aria-label="Sepet"><ShoppingBag/><b>{count}</b></button></div>
  </header>
