...
export async function getBrands(){
 ...
 return rows?.length?rows.map(row=>({id:row.id,slug:row.slug,name:row.name,copy:row.description||'',logo:row.logo_url||null,metaTitle:row.meta_title,metaDescription:row.meta_description})):fallbackBrands;
}
